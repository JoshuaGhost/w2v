...
from numpy import array
from utils import dim_reduce, low_rank_align
...
out_fname = Path('./lra_comb_multiplication_eva.csv').resolve()
...
if __name__=='__main__':
    ...
    for n_subs in n_subs_list:
        ...
        for i in xrange(n_comb):
            if n_subs == n_subs_total:
                idx_list = [idx for idx in range(n_subs_total)]
            else:
            ...
            vecs_merge = vecs[:, idx_list[0] * dim_sub : (idx_list[0] + 1) * dim_sub]
            for nmerged, sub_idx in enumerate(idx_list[1:]):
                ...
                _, Fx = dim_reduce(None, Fx, dim_sub, False)
                _, Fy = dim_reduce(None, Fy, dim_sub, False)
                vecs_merge = Fx * Fy
