...
class Embedding(object):
    ...
    def from_gensim(model):
        ...
        for word, vocab in sorted(iteritems(model.wv.vocab), key=lambda item: -item[1].count):
            ...
            if word and not word in word_count.keys():
                vectors.append(model.wv.syn0[vocab.index])
    ...
    @staticmethod
    def to_csv(w, fname):
        logger.info('storing {}x{} projection weight into {}'.format(w.vectors.shape[0], w.vectors.shape[1], fname))
        with _open(fname, 'w+') as fout:
            for word, vector in zip(w.vocabulary.words, w.vectors):
                fout.write(to_utf8('{},{}'.format(word, ','.join("%.15f" % val for val in vector))))
